...
class QuizController extends Controller
{
    ...
    public function quizlisting($lecture_id="",$topic_id="",$course_id=""){
    ...
    public function addquizquestions($id=""){
        ...
        $quiz_id = $id;
        $questions = TopicQuizQuestions::select('*')->where('quiz_id',$quiz_id)->get();
        return view('admin.addquizquestions',compact('quiz_id','questions'));
    }
    
    public function listquizquestions($id=""){
        
        $quiz_id = $id;
        $questions = TopicQuizQuestions::select('*')->where('quiz_id',$quiz_id)->get();
        $quiz = TopicQuiz::find($quiz_id);
        $topic_id = $quiz['topic_id'];
        return view('admin.quizquestionslist',compact('quiz_id','questions','topic_id'));
    }

    public function addquizoptions($id=""){
        
        $quizid = $id;
        $quiz = TopicQuiz::find($quizid);
        $topic_id = $quiz['topic_id'];
        $allQuiz = TopicQuiz::where('topic_id',$topic_id)->get();
        return view('admin.addquizoptions',compact('quizid','allQuiz','topic_id'));
    }
    ...
    public function savetopicquestions(Request $request){
            //pr($request->all());
            $topicQuizQuestions = new TopicQuizQuestions;
            $validator = Validator::make($request->all(),  [
                 'questiontype' => 'required',
                 'questionarea' => 'required',
                 'ans_option' => 'required'
        ...
        if($request->questiontype == '1'){
            $topicQuizQuestions->optionA = $request->option_a;
            $topicQuizQuestions->optionB = $request->option_b;
            $topicQuizQuestions->optionC = $request->option_c;
            $topicQuizQuestions->optionD = $request->option_d;
        }else{
            $topicQuizQuestions->optionA = $image_name[0];
            $topicQuizQuestions->optionB = $image_name[1];
            $topicQuizQuestions->optionC = $image_name[2];
            $topicQuizQuestions->optionD = $image_name[3];
        }
        $topicQuizQuestions->topic_id = $request->chapter_id;
        $topicQuizQuestions->quiz_id = $request->quiz_id;
        $topicQuizQuestions->question = $request->questionarea;
        $topicQuizQuestions->answer = $request->ans_option;
        $topicQuizQuestions->questionImage = $request->quest_image;
        $topicQuizQuestions->questionLink = $request->quest_video;
        $topicQuizQuestions->questionType = $request->questiontype;
        $topicQuizQuestions->save();
        Session::flash('message', ' המשתמש נוסף בהצלחה. ');
        return response()->json(['success' => true]);
         }else{
        return response()->json(['error'=>$validator->errors()]);
    ...
    public function editTopicQuestion($id="",$chapterId =""){
        
        $questions_data = TopicQuizQuestions::select('*')->where('id',$id)->get();
        foreach ($questions_data as $key => $value) {
            $question_data = $value;
        }
        $allQuiz = TopicQuiz::select('id','quiz_title','topic_id')->where('topic_id',$chapterId)->get()->toArray();
        return view('admin.editTopicQuestion',compact('question_data','chapterId','id','allQuiz'));
    }
    public function updateTopicQuestion(Request $request){
        ...
        //$quiz_questions = quiz_questions::findOrFail($id);
        $old_question_data = TopicQuizQuestions::find($id);
        $quiz_questions = TopicQuizQuestions::findOrFail($id);
